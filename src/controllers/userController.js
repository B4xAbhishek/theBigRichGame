...
const aviator = async (req, res) => {
   let auth = req.cookies.auth
   res.redirect(`http://jetx.asia/theninja/src/api/userapi.php?action=loginandregisterbyauth&token=${auth}`)
   // res.redirect(`https://999club.site/theninja/src/api/userapi.php?action=loginandregisterbyauth&token=${auth}`);
...
//     const [bonus] = await connection.query("SELECT * FROM users WHERE `phone` = ? AND status = 1", [rows[0].phone])
//   let totalBonus = 0
//   bonus.forEach(data => {
